...
describe('web onboarding tool result helpers', () => {
  ...
  it('formats onboarding state as a plain-language summary', () => {
    ...
    expect(message).toContain(
      'Questioning rule: prefer the `lobe-user-interaction________builtin` tool call for structured collection or explicit UI input. For natural exploratory questions, plain text is allowed.',
    );
  });
